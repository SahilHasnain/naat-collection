...
export default function ExcludeNaatsClient() {
  ...
  async function loadNaats() {
    try {
      ...
      // Fetch all naats with pagination
      let allNaats: Naat[] = [];
      let hasMore = true;
      let offset = 0;
      const limit = 100;
      ...
      while (hasMore) {
        ...
        allNaats = [...allNaats, ...(response.documents as unknown as Naat[])];
  ...
  const filteredNaats = naats
    .filter((naat) => {
      const matchesSearch =
        naat.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
        naat.youtubeId.toLowerCase().includes(searchTerm.toLowerCase()) ||
        naat.channelName?.toLowerCase().includes(searchTerm.toLowerCase());

      const matchesFilter =
        filterExcluded === "all" ||
        (filterExcluded === "excluded" && naat.exclude) ||
        (filterExcluded === "included" && !naat.exclude);
    ...
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-gray-800 rounded-lg p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div className="bg-gray-700 rounded-lg p-4">
              <p className="text-sm text-gray-400">Total Naats</p>
              <p className="text-2xl font-bold">{naats.length}</p>
            </div>
            ...
            <div className="bg-red-900/30 border border-red-700 rounded-lg p-4">
              ...
              <p className="text-2xl font-bold text-red-400">{excludedCount}</p>
            </div>
          </div>
        ...
        <div className="mt-6 text-sm text-gray-400 text-center">
          Showing {filteredNaats.length} of {naats.length} naats
        </div>
      </div>
    </div>
  );
